...
fractal の14秒放置で自動ツアー開始、lightning の常時降雨+自動ストーム、slime の
リサージュ仮想フィーダー、metro の常時運行+路線紹介ループ、solar の惑星巡りツアー）は
この要件から来ているので、壊さないこと。
...
**Browser ペインのスクリーンショットも当てにならない。** 合成（コンポジット）が止まって
いるため、起動画面など昔のフレームが写り続ける。見た目を確認するときは、`update()` を
進めたのと**同じ JS 実行内**で canvas を 2D キャンバスへ `drawImage` → `toDataURL` し、
その JPEG をデコードして見る。`readPixels` も同一タスク内でのみ有効で、実行をまたぐと
drawing buffer はクリア済みの真っ黒を返す（preserveDrawingBuffer が false のため）。
...
- **three.js は `position` 属性の無い非インデックスジオメトリを1頂点も描かない。**
  描画数を position から数えるため。カスタム属性＋頂点シェーダだけで完結する描画でも、
  ゼロ埋めの position を付けること（lightning の雨が前例。エラーは一切出ない）。
- **頂点シェーダで太らせるクアッドリボンは `side: DoubleSide` にする。** 三角形の巻き
  方向が線分の向きで反転するため、FrontSide のままだとバックフェースカリングで
  静かに全滅する（lightning の稲妻が前例。draw call は正常に出るので気づきにくい）。
